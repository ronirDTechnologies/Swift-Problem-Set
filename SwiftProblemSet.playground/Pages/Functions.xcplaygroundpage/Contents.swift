...
func emojiLove(s1 stringParmeterOne: String, s2 stringParameterTwo: String) {
    print("\(stringParmeterOne) ❤️ \(stringParameterTwo)")
}
...
{
    var intParamArray = [num1, num2, num3]
    intParamArray.sort()
    
    return intParamArray[1]
}
 ...
 */
func beginsWithVowel(_ nameOfString: String) -> Bool {
    ...
    {
        let firstCharOfString = nameOfString.lowercased().characters[nameOfString.startIndex]
        if vowels.contains(firstCharOfString)
        {
            return true
        }
        else
 ...
 */
func funWithWords(_ wordsParam: String) -> String {
    var formattedWord: String
    if (beginsWithVowel(wordsParam))
    {
        formattedWord = wordsParam.uppercased()
    }
    else
    {
        formattedWord = wordsParam.lowercased()
    }
